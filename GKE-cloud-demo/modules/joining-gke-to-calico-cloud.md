# Module 1: Joining GKE cluster to Calico Cloud

**Goal:** Join GKE cluster to Calico Cloud management plane.
...
3. Join AKS cluster to Calico Cloud management plane.
    ...
    choose GKE and click next
      ![choose-aks](../img/choose-gke.png)


    run installation script in your gke cluster. 
    ```bash
    ...
    Your Connected Cluster Name is r69bq5ox-management-managed-34-75-84-30  
    ```
    Set the Calico Cluster Name as a variable to use later in this workshop. The Cluster Name can also be obtained from the Calico Cloud Web UI at a later date. For the example above `CALICOCLUSTERNAME` should be set to 
    __r69bq5ox-management-managed-34-75-84-30__
    ...
    In calico cloud management UI, you can see your own gke cluster added in "managed cluster", you can also confirm by
    ```bash
...
5. Configure log aggregation and flush intervals in gke cluster, we will use 60s instead of default value 300s for lab testing only.   
